package com.chenshuhong.blog.swagger.plugin;

import com.fasterxml.jackson.databind.introspect.AnnotatedField;
import com.chenshuhong.blog.valid.EnumInteger;
import com.chenshuhong.blog.valid.EnumString;
    ...
    public static <A, P> void apply(A annotationObject, BiFunction<A, Class, Annotation> getAnnotation,
        ...
        if (annotationEnumInteger != null && inGroups(annotationEnumInteger.groups(), groups)) {
            allowableValues.accept(propertyObject, new AllowableListValues(
                    Arrays.stream(annotationEnumInteger.value()).mapToObj(v -> Integer.toString(v)).collect(Collectors.toList()),
                    "int"
            ));
        }
